...
const PUBLIC_KEY_B64 = "LS0tLS1CRUdJTiBQVUJMSUMgS0VZLS0tLS0KTUNvd0JRWURLMlZ3QXlFQWdBRldDTklHL0x4L01YNng0Ykdac3lNY0w1aU14Z1hnOUNtQ2Fqc0JuMDA9Ci0tLS0tRU5EIFBVQkxJQyBLRVktLS0tLQo=";
let _publicKey = null;
...
export function parseLicense(key) {
    ...
    const valid = verify(null, payload, getPublicKey(), signature);
    if (!valid)
        return null;
    let data;
    try {
    ...
    if (!data.email ||
        ...
        !data.issued ||
        !data.expires ||
        !VALID_TIERS.includes(data.tier)) {
        return null;
    }
    // Check expiry
...
export function printLicenseBanner() {
    const key = process.env.LICENSE_KEY;
    const result = validateLicense(key);
    if (result.valid && result.data) {
        console.error(`[License] 已激活: ${result.data.email} | 等级: ${result.data.tier} | 到期: ${result.data.expires}`);
    }
    else {
