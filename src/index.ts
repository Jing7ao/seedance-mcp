...
interface UsageStats {
  ...
  thisMonth: number;
  month: number;
  history: Array<{ date: string; taskId: string; duration: number; resolution: string }>;
}
...
function trackUsage(taskId: string, duration: number, resolution: string) {
  const now = new Date();
...
server.tool(
  "text_to_video",
  "用文字描述生成 AI 视频（文生视频）。支持 5-15 秒，多种分辨率和画面比例。",
  {
...
server.tool(
  "get_usage_stats",
  "查询当前账号的 Seedance 视频生成用量统计（本月/累计）。用于用量监控和计费。",
  {},
  async () => {
    const usage = loadUsage();
    return {
      content: [
        {
          ...
          text: [
            ...
            ...usage.history.slice(-5).reverse().map(
              (h) => `  ${h.date.slice(0, 10)} | ${h.taskId} | ${h.duration}s ${h.resolution}`
            ),
          ].join("\n"),
        },
      ],
